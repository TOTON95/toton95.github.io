...
I just needed someone to test it on their computer to confirm that the same result could be reproduced, so I decided to send it to my friend [Evan Krell](https://github.com/ekrell) from Texas A&M University - Corpus Christi, who confirmed it worked, made practical use of the program in his project and became a contributor to the repository, with all that evidence I made a Tweet with the current status of the code... and the community loved it. 🖤
